...
def testDetail():
    try:
        ...
        query1 = 'select pm.PatientId from Patient_Vital_master as pvm,Patient_master as pm where pm.PatientID=pvm.Patient_Id and pm.PatientId="'+str(data['PatientId'])+'"'
        conn = Connection()
        cursor = conn.cursor()
        cursor.execute(query1)
        data1 = cursor.fetchone()
