...
def testDetail():
    try:
        ...
        query1 = 'select PatientId from Patient_Vital_master as pvm,Patient_master as pm where pm.PatientID=pvm.Patient_Id and pvm.PatientId="'+str(data['PatientId'])+'"'
        conn = Connection()
        cursor = conn.cursor()
        cursor.execute(query1)
        data1 = cursor.fetchone()
        print('11111112332434')
        if data1:
            query2 = 'select * from Patient_Vital_master as pvm where pvm.PatientId="'+str(data['PatientId'])+'"'
            conn = Connection()
            cursor = conn.cursor()
