...
def getPatientList():
    ...
    try:
        ...
        query1 = 'select pdm.ID,pdm.PatientId,pdm.doctorId from patientDoctorMapping as pdm,PatientMaster as pm where pdm.Patient_Id=pm.ID and pdm.doctorId="'+str(data['doctorId'])+'"'
        conn = Connection()
        cursor = conn.cursor()
        cursor.execute(query1)
        data2 = cursor.fetchall()
