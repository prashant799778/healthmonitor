...
return(<DetailStyled>
  ...
  { textlowPressure!="" &&    <h3 className="erroe-mssh" style={{background:'red'}}>{textlowPressure}</h3>}
               ...
               { textheartRate!="" &&    <h3 className="erroe-mssh" style={{background:'#b030b0'}} >{textheartRate}</h3>}
				...
				<div class="dropdown">
				{localStorage.getItem("user_type","Operation")!="Operation" &&  <span onClick={()=>{this.changemsgState()}} > <button class="side-button-pis" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">prescribe medicine</button></span>}
